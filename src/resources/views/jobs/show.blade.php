                ...
                <div class="p-4">
                    ...
                    <ul class="my-4 bg-gray-100 p-4">
                        ...
                        <li class="mb-2">
                            ...
                            <span
                                class="text-xs {{ $job->is_remote ? 'bg-red-500' : 'bg-green-500' }} text-white rounded-full px-3 py-1 ml-2">
            ...
            <div class="container mx-auto p-4">
                @if ($job->requirements || $job->benefits)
                    ...
                    <div class="rounded-lg shadow-md bg-white p-4">
                        ...
                        <h3 class="text-lg font-semibold mb-2 mt-4 text-blue-500">Benefits</h3>
                        <p>{{ $job->benefits }}</p>
                    </div>
                @endif
                <p class="my-5">
                    Put "Job Application" in the subject line and email your resume to
                    <a href="mailto:{{ $job->contact_email }}"
                        class="block text-center px-5 py-2.5 shadow-sm rounded border text-base font-medium cursor-pointer text-indigo-700 bg-indigo-100 hover:bg-indigo-200">{{ $job->contact_email }}</a>
                    or apply online at
                    <a href="{{ $job->company_website }}"
                        class="text-blue-500 underline">{{ $job->company_website }}</a>
                </p>
            </div>
        </section>
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            @if ($job->company_logo)
                <img src="/storage/{{ $job->company_logo }}" alt="{{ $job->company_name }}"
                    class="w-fullrounded-lg mb-4 m-auto">
            @endif
